...
if platform.system() == "Windows":
    clib = ctypes.CDLL(os.path.join(this_dir, 'lib.dll'))  # Windows 上使用 .dll 文件
elif platform.system() == "Darwin":  # macOS
    clib = ctypes.CDLL(os.path.join(this_dir, 'lib.dylib'))
else:
    clib = ctypes.CDLL(os.path.join(this_dir, 'lib.so'))  # Linux 上使用 .so 文件
