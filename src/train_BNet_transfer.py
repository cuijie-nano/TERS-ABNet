...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "FG_CNO05A_H09A_bondmapping_decay5.npy"
        ],
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "QM_CNO05A_H09A_bondmapping_decay5.npy"
        ],
    )
    ...
    parser.add_argument(
        "--savedir",
        type=str,
        default="../checkpoints/BNet_transfer/",
    )
    ...
    model = AttUnet(
        ...
        # 加载预训练权重
    pretrained_dict = torch.load(args.predir+"model100.pth")
    ...
    # 处理conv2d_12的权重（部分复用）
    if 'conv2d_12.weight' in pretrained_dict:
        old_weight = pretrained_dict['conv2d_12.weight']
        new_weight = torch.randn(11, 64, 3, 3)
        new_weight[2:11, :, :, :] = old_weight[:, :, :, :]  # 复用前18个通道
        matched_weights['conv2d_12.weight'] = new_weight
        
        if 'conv2d_12.bias' in pretrained_dict:
            old_bias = pretrained_dict['conv2d_12.bias']
            new_bias = torch.zeros(11)
            new_bias[2:11] = old_bias[:]
            matched_weights['conv2d_12.bias'] = new_bias
